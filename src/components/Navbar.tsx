...
function NavbarContent() {
  ...
  const NavItemWithDropdown = ({ title, id, items, alignRight = false }: { title: string, id: string, items: any[], alignRight?: boolean }) => (
    ...
    >
      <button className={`flex items-center gap-1 hover:underline decoration-2 underline-offset-8 outline-none ${id === 'business' ? styles.mainLinkClass : styles.textColorClass}`}>
        {title}
  ...
  return (
    <>
      <nav className={`fixed top-0 z-50 w-full transition-all duration-300 px-4 md:px-10 py-3 md:py-4 ${styles.navBgClass}`}>
        <div className="flex items-start justify-between gap-2 md:gap-0">
          ...
          <div className="flex-shrink-0 h-[48px] md:h-[66px] flex items-center">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className={`hidden md:block text-2xl font-bold tracking-tight ${styles.logoTextClass}`}>
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-2 md:px-0">
             ...
             <div className="hidden md:flex gap-8 text-sm font-bold mt-2 transition-colors duration-300">
          ...
          <div className="hidden md:flex flex-shrink-0 h-[66px] items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              <button onClick={openLogin} className={`px-5 py-2.5 text-sm font-bold rounded-full border-2 transition-all ${isScrolled ? 'border-gray-900 text-gray-900 hover:bg-gray-100' : 'border-white/80 text-white hover:bg-white/20'}`}>Log In</button>
          ...
          <div className="flex md:hidden flex-shrink-0 h-[48px] items-center">
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className={`p-2 rounded-full ${isScrolled || mobileMenuOpen ? 'text-gray-900 bg-gray-100' : 'text-white bg-white/20'}`}>
        ...
        {/* MOBILE MENU */}
        {mobileMenuOpen && (
          <div className="absolute top-[100%] left-0 w-full bg-white shadow-xl border-t border-gray-100 flex flex-col p-5 animate-in slide-in-from-top-2 md:hidden max-h-[80vh] overflow-y-auto">
            {/* We just need one generic Icon for the Section Header in mobile, 
                or we can grab the first icon from the list, or keep importing main icons.
                For simplicity, I reused the first item's icon logic or you can import generic icons like Building2 here. 
            */}
            ...
            <div className="flex flex-col gap-3 py-2 text-sm font-semibold text-gray-600">
...
// Updated Mobile Section to use the icons from the config items
const MobileSection = ({ title, items }: any) => {
  // Use the icon of the first item as the "Section Icon" (or you can pass a specific one)
    ...
    <div className="mb-4">
      ...
      <div className="pl-7 flex flex-col gap-2">
          {items.map((i: any, idx: number) => {
             ...
             return (
               <Link key={idx} href={i.href} className="flex items-center gap-2 text-gray-600 text-sm font-medium hover:text-pink-600">
