...
const CustomExperienceBuilder: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  venuePrice: number;
}> = ({ isOpen, onClose, venuePrice }) => {
    ...
    return (
      <div className="bg-background min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-white/40 text-center px-6">
          ...
          <button
            onClick={() => router.back()}
            className="mt-2 px-4 py-2 rounded-full border border-white/10 text-xs font-bold text-white hover:bg-white hover:text-black transition-all"
          >
            Go back
          </button>
        </div>
