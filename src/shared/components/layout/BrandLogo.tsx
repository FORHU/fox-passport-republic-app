...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  return (
    <Link
      href="/"
      className={`flex items-center gap-3 group cursor-pointer relative ${className}`}
    >
      <div
      ...
      <div className="relative">
        <h2
          className={`${textSize} font-display font-bold tracking-tight ${textBaseClass} ${hoverTextClass} transition-colors`}
        >
          FoxPassport
        </h2>
        <span
          className={`absolute bottom-0 left-0 w-0 h-0.5 ${underlineBgClass} group-hover:w-full transition-all duration-300`}
        ></span>
      </div>
    </Link>
  );
