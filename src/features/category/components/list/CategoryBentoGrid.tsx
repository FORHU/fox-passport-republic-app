...
export const CategoryBentoGrid: React.FC<CategoryBentoGridProps> = ({ categories, onCategoryClick, searchQuery }) => {
  if (categories.length === 0) {
    return (
      <div className="col-span-full py-12 sm:py-20 text-center">
        <div className="text-4xl sm:text-6xl text-slate-200 mb-4">🔍</div>
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 pb-24 auto-rows-[160px] sm:auto-rows-[200px] lg:auto-rows-[240px]">
      {categories.map((cat, index) => {
        ...
        return (
          <div
            ...
            className={`group relative rounded-xl sm:rounded-[2rem] overflow-hidden cursor-pointer shadow-lg transition-transform duration-500 hover:-translate-y-1 ${gridSpan}`}
          >
            ...
            <div className="absolute bottom-0 left-0 p-4 sm:p-6 lg:p-8 w-full flex justify-between items-end">
              <div className="flex-1 min-w-0">
                <h3 className={`${isLarge ? 'text-lg sm:text-2xl lg:text-3xl' : 'text-sm sm:text-lg lg:text-xl'} font-bold text-white mb-0.5 sm:mb-1 group-hover:text-pink-200 transition-colors truncate`}>
                  {cat.name}
                </h3>
                <p className="text-white/80 text-[10px] sm:text-xs lg:text-sm font-medium line-clamp-2">
                  {cat.description || "Explore experiences"}
                </p>
              </div>
              {isLarge && (
                <div className="hidden sm:flex w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-pink-500 items-center justify-center text-white shadow-[0_0_20px_rgba(236,72,153,0.4)] opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all shrink-0">
