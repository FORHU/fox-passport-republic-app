  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawVenues =
          venueResp.data.venues ||
          venueResp.data.data ||
          (Array.isArray(venueResp.data) ? venueResp.data : []);

        if (rawVenues && Array.isArray(rawVenues)) {
          venues = rawVenues
            .filter(
              (v: Record<string, unknown>) =>
                (v.status as string)?.toLowerCase() === "available",
            )
            .map((v: Record<string, unknown>) => ({
              id: String(v.id),
              name: String(v.name ?? ""),
              cost: Number(
                (v.price as number) || (v.baseRate as number) || 10000,
              ),
              icon: "location_city",
        ...
        if (assetResp.data.assets) {
          assetResp.data.assets.forEach((a: any) => {
            ...
            if (slug === "entertainment") {
              ...
              ["decorations", "decoration", "furnitures", "furniture"].includes(
                slug,
              )
            ) {
        ...
        // Process Services
        const rawServices =
          serviceResp.data.services || serviceResp.data.data || [];
        if (Array.isArray(rawServices)) {
          rawServices.forEach((s: any) => {
            ...
            if (category === "entertainment") {
              ...
              service.push({
                ...item,
                icon: iconMap[category] || "work_outline",
              });
            } else {
              service.push(item);
            }
          ...
          try {
            ...
            for (const vId of existingVenueIds) {
              if (vId !== desiredVenue?.id) {
                try {
                  await api.delete(
                    `/event-templates/${templateId}/venues/${vId}`,
                  );
                } catch (e) {
                  console.error("[syncInclusions] remove venue failed", e);
                }
              }
            }
            if (desiredVenue && !existingVenueIds.includes(desiredVenue.id)) {
              try {
                ...
                console.error(
                  "[syncInclusions] attach venue failed",
                  e?.response?.data ?? e,
                );
              }
            }
            ...
            for (const aId of existingAssetIds) {
              if (!desiredAssetIds.includes(aId)) {
                try {
                  await api.delete(
                    `/event-templates/${templateId}/assets/${aId}`,
                  );
                } catch (e) {
                  console.error("[syncInclusions] remove asset failed", e);
                }
              }
            }
            for (const asset of desiredAssets) {
              if (!existingAssetIds.includes(asset.id)) {
                try {
                  ...
                  console.error(
                    "[syncInclusions] attach asset failed",
                    asset.name,
                    e?.response?.data ?? e,
                  );
                }
            ...
            // ── Services ─────────────────────────────────────────────────────────
            const existingServiceIds: string[] = (
              current?.templateServices ?? []
            )
              .map((ts: any) => ts.serviceId)
              .filter(Boolean);
            // Fallback: treat items with service icons as services when resourceType is missing
            const desiredServices = store.baseItems.filter(
              (i) =>
            ...
            for (const sId of existingServiceIds) {
              if (!desiredServiceIds.includes(sId)) {
                try {
                  await api.delete(
                    `/event-templates/${templateId}/services/${sId}`,
                  );
                } catch (e) {
                  console.error("[syncInclusions] remove service failed", e);
                }
              }
            }
            for (const service of desiredServices) {
              if (!existingServiceIds.includes(service.id)) {
                try {
                  ...
                  console.error(
                    "[syncInclusions] attach service failed",
                    service.name,
                    e?.response?.data ?? e,
                  );
                }
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      const response = store.draftId
        ? await api.put(`/event-templates/${store.draftId}`, {
            ...payload,
            isPublic: false,
          })
