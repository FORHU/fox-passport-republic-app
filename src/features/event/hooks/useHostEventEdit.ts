...
function eventAssetToResourceItem(a: any) {
  ...
  if (slug === "entertainment") icon = "music_note";
    ...
    icon = slug === "furniture" ? "chair" : "brush";
...
function eventServiceToResourceItem(s: any) {
  ...
  if (slug === "entertainment") icon = "music_note";
    ...
    ["planning", "catering", "photography", "videography"].includes(slug)
  ) {
...
export function useHostEventEdit(eventId: string) {
  ...
  const submitEvent = async (targetStatus: string) => {
    ...
    try {
      ...
      const rawCategory = builder.category
        .toLowerCase()
        .replace(/\s+/g, "_")
        .replace(/_/g, " ");
      ...
      const payload: Record<string, unknown> = {
        ...
        isPublic: false,
        maxAttendees:
          builder.maxAttendees > 0 ? builder.maxAttendees : undefined,
      ...
      try {
        ...
        if (Array.isArray(images)) {
          images.forEach((img: any, idx: number) => {
            ...
            if (!url) return;
            const caption =
              img?.altText ?? img?.caption ?? img?.name ?? `Image ${idx + 1}`;
        ...
        if (venue) {
          builder.addBaseItem({
            ...
            cost:
              Number(
                venue?.baseRate ?? venue?.price ?? found?.venuePrice ?? 0,
              ) || 0,
            icon: VENUE_ICONS[0],
