...
export default function FulfillmentPassClient({
  bookingType,
  bookingId,
}: Props) {
  ...
  useEffect(() => {
    const fetcher =
      bookingType === "service" ? fetchServiceBooking : fetchAssetBooking;
    fetcher(bookingId)
      .then(setBooking)
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <span className="animate-spin material-symbols-outlined text-accent text-4xl">
          progress_activity
        </span>
      </div>
    );
  ...
  if (error || !booking) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-center px-4">
        <div>
          <span className="material-symbols-outlined text-white/20 text-6xl block mb-4">
            error
          </span>
  ...
  const location = isService
    ? booking.location
    : booking.deliveryAddress || "Pickup from owner";
          ...
          <div className="glass-panel rounded-3xl p-8 border border-white/10">
            <div className="flex items-start justify-between gap-4 mb-6">
              <div className="min-w-0">
                <h1 className="text-3xl font-display font-bold text-white truncate mb-1">
                  {item?.name}
                </h1>
                <p className="text-xs font-bold text-accent uppercase tracking-[3px]">
                  FULFILLMENT PASS #{passId}
                </p>
              </div>
              <span
                className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border ${statusBadge}`}
              >
                {status}
              </span>
            </div>
              ...
              <div className="mt-5 pt-4 border-t border-white/10 space-y-2.5">
                ...
                {!isService && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white font-bold">
                      {booking.quantity} unit(s) · {booking.fulfillmentType}
                    </span>
                  </div>
                )}
            ...
            <div className="glass-panel rounded-3xl p-6 border border-white/10 relative overflow-hidden">
                  ...
                  >
                    <div
                      className={`h-4 w-4 rounded-full border flex items-center justify-center shrink-0 transition-all ${
                          ...
                          ? "bg-accent border-accent shadow-[0_0_8px_rgba(204,255,0,0.4)]"
                    ...
                    <p
                      className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${done[i] ? "text-white" : "text-white/30"}`}
                    >
                      {step.label}
                    </p>
                  </div>
              ...
              {isDisputed ? (
                <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4 text-center">
                  <span className="material-symbols-outlined text-red-400 text-3xl block mb-1">
                    warning
                  </span>
                  ...
                  <p className="text-white/40 text-xs mt-1">
                    Our team will review within 24 hours. You&apos;ll be
                    contacted by email.
                  </p>
                </div>
              ) : isCompleted ? (
                <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-4 text-center">
                  <span className="material-symbols-outlined text-green-400 text-3xl block mb-1">
                    check_circle
                  </span>
                  ...
                  <p className="text-white/40 text-xs mt-1">
                    Payment has been wired to the provider.
                  </p>
                </div>
              ) : isActive ? (
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-2xl p-4 text-center">
                  <span className="material-symbols-outlined text-blue-400 text-3xl block mb-1">
                    send_money
                  </span>
                  ...
                  <p className="text-white/40 text-xs mt-1">
                    Funds are being released. 3–5 business days.
                  </p>
                </div>
              ) : canConfirm ? (
                <>
                  ...
                  <p className="text-[10px] text-amber-400/80 text-center mt-2 font-medium uppercase tracking-wider leading-relaxed">
                    Important: Only confirm once they arrive. Your money is 100%
                    safe in our vault until this step.
                  </p>
                </>
              ) : (
          ...
          <div className="glass-panel rounded-3xl p-8 border border-white/10 max-w-sm w-full animate-in slide-in-from-bottom-4 duration-300">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-text-muted leading-relaxed">
                This flags the booking for admin review. If the provider truly
                failed to show, we&apos;ll initiate a full refund within 3–5
                business days.
              </p>
            </div>
