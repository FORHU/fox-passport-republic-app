...
export default function BookingConfigurationClient() {
  ...
  const templateCategory = template?.category
    ? template.category.charAt(0).toUpperCase() +
      template.category.slice(1).toLowerCase().replace(/_/g, " ")
    : "Event";
  ...
  const optionalAssets: any[] = (template?.templateAssets ?? []).filter(
    (ta: any) => ta.isOptional,
  );
  const optionalServices: any[] = (template?.templateServices ?? []).filter(
    (ts: any) => ts.isOptional,
  );
  const optionalVenues: any[] = (template?.templateVenues ?? []).filter(
    (tv: any) => tv.isOptional,
  );
  const allOptionalItems: any[] = [
    ...optionalAssets.map((ta) => ({
      ...ta,
      _type: "asset",
      label: ta.asset?.name ?? "Gear",
      price: ta.agreedPrice,
    })),
    ...optionalServices.map((ts) => ({
      ...ts,
      _type: "service",
      label: ts.service?.name ?? "Service",
      price: ts.agreedPrice,
    })),
    ...optionalVenues.map((tv) => ({
      ...tv,
      _type: "venue",
      label: tv.venue?.name ?? "Venue",
      price: tv.agreedPrice,
    })),
  ];
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 text-sm text-text-muted mb-4">
              ...
              <span className="text-accent font-semibold">
                {isLoadingTemplate ? "…" : templateName}
              </span>
            </div>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <p className="text-text-muted">
                  Customize your experience and get ready for the core memory.
                </p>
              </div>
              <div className="flex items-center gap-4 bg-surface-highlight/30 px-6 py-3 rounded-full border border-white/5">
                <div className="flex items-center gap-2">
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-accent text-black text-xs font-bold">
                    1
                  </span>
                ...
                <div className="glass-card rounded-2xl sm:rounded-[2rem] p-3 sm:p-8 border border-yellow-400/20 text-center">
                  <div className="flex flex-col items-center gap-4 py-8">
                    ...
                    <p className="text-text-muted max-w-md">
                      All spots are currently filled. Join the waitlist to be
                    ...
                    {isLoadingTemplate ? (
                      <div className="grid sm:grid-cols-2 gap-4">
                        {[1, 2, 3, 4].map((i) => (
                          <div
                            key={i}
                            className="p-5 rounded-2xl bg-surface-highlight/30 border border-white/5 h-28 animate-pulse"
                          />
                        ))}
                      </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        {includedServices.map((ts: any, i: number) => {
                          ...
                          return (
                            <div
                              key={ts.id ?? i}
                              className="p-5 rounded-2xl bg-surface-highlight/30 border border-accent/30 bg-accent/5 h-full"
                            >
                              <div className="flex justify-between items-start mb-3">
                                <div
                                  className={`h-10 w-10 rounded-lg bg-linear-to-br ${gradients[i % gradients.length]} flex items-center justify-center text-white shadow-lg`}
                                >
                      ...
                      <p className="text-text-muted text-sm">
                        All services included in the package price.
                      </p>
                    )}
                  </div>
                  ...
                  {allOptionalItems.length > 0 && (
                    <div className="glass-card rounded-2xl sm:rounded-[2rem] p-3 sm:p-8 border border-yellow-400/20">
                      ...
                      <p className="text-xs text-white/40 mb-6">
                        Uncheck any item you don&apos;t need. The price will
                      ...
                      <div className="space-y-4">
                        <div className="rounded-2xl bg-yellow-400/10 border border-yellow-400/20 p-4 text-center">
                          ...
                          <p className="text-xs text-white/50 mt-1">
                            Join the waitlist for a chance to get in if a spot
                            opens up.
                          </p>
                        </div>
                              ...
                              try {
                                ...
                                const result = await bookFromTemplate({
                                  ...
                                  excludedAssetIds: optionalAssets
                                    .filter((ta) => excludedItemIds.has(ta.id))
                                    .map((ta) => ta.id),
                                  excludedServiceIds: optionalServices
                                    .filter((ts) => excludedItemIds.has(ts.id))
                                    .map((ts) => ts.id),
                                  excludedVenueIds: optionalVenues
                                    .filter((tv) => excludedItemIds.has(tv.id))
                                    .map((tv) => tv.id),
                                });
                  ...
                  <div className="p-6 space-y-0">
                    {[
                      ...
                      },
                      {
                        ...
                        desc: "On event day, confirm their arrival to release payment.",
                      },
                    ].map((step, i) => (
                      <div key={i} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          <div
                            className={`h-9 w-9 rounded-xl flex items-center justify-center shrink-0 ${i === 0 ? "bg-accent text-black" : "bg-white/5 border border-white/10 text-white/40"}`}
                          >
                        ...
                        <div className="pb-5">
                          <p
                            className={`text-sm font-bold mb-0.5 ${i === 0 ? "text-accent" : "text-white/70"}`}
                          >
                            {step.title}
                          </p>
