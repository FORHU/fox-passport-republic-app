...
export const AdminAssetsTable: React.FC<AssetsTableProps> = ({
  assets,
  isLoading,
}) => {
  ...
  const [rejectingAsset, setRejectingAsset] = useState<{
    id: string;
    name: string;
  } | null>(null);
                    ...
                    <tr className="border-b border-white/5 hover:bg-white/3 transition-colors group">
                      ...
                      <td className="p-6">
                        <p className="font-bold text-white/90">
                          {asset.owner?.name || asset.user?.name || "—"}
                        </p>
                        <p className="text-[10px] text-white/30">
                          ID:{" "}
                          {(asset.ownerId || asset.userId || "").slice(0, 8)}...
                        </p>
                      </td>
                      <td className="p-6">
                      ...
                      <td className="p-6">
                        <button
                          onClick={() =>
                            setExpandedRow(
                              expandedRow === asset.id ? null : asset.id,
                            )
                          }
                        ...
                        >
                          ...
                          <span
                            className={`material-symbols-outlined text-[14px] transition-transform duration-300 ${expandedRow === asset.id ? "rotate-180" : ""}`}
                          >
                            expand_more
                          </span>
                        </button>
                      ...
                      <td className="p-6 text-right">
                        <div className="flex justify-end items-center gap-2">
                          ...
                          <button
                            disabled={!!updatingId}
                            onClick={() =>
                              openRejectDialog(asset.id, asset.name)
                            }
                        ...
                        >
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-6 rounded-3xl bg-black/40 border border-white/5">
                            <div className="col-span-2">
                              <p className="text-[9px] uppercase font-bold text-purple-400 tracking-[0.2em] mb-2">
                                Description
                              </p>
