...
export const AdminChartsSection: React.FC<Props> = ({
  bookingsByDay,
  categoryStats,
}) => {
        ...
        {isEmpty ? (
          ...
          <div className="h-48 sm:h-64 flex items-end justify-between gap-1.5 sm:gap-3 lg:gap-4 px-1 sm:px-2">
            {bookingsByDay.map((count, i) => {
              const pct =
                maxDay > 0
                  ? Math.max(4, Math.round((count / maxDay) * 100))
                  : 4;
              ...
              return (
                <div
                  ...
                  title={`${DAY_LABELS[i]}: ${count} bookings`}
                >
                  <div
                  ...
                  >
                    <div
                      className={`absolute inset-0 bg-linear-to-t ${isToday ? "from-accent/50 to-accent/80 opacity-80" : "from-primary/50 to-primary/80 opacity-60"} group-hover:opacity-100 transition-opacity ${isToday ? "shadow-[0_0_20px_rgba(204,255,0,0.2)]" : ""}`}
                    />
                    {count > 0 && (
                  ...
                  <span
                    className={`text-xs text-center ${isToday ? "text-white font-bold" : "text-gray-500 font-medium"}`}
                  >
                    {DAY_LABELS[i]}
                  </span>
                </div>
        ...
        {topCats.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-white/20">
            ...
            <p className="text-sm text-center">
              Category data will appear once events are created.
            </p>
          </div>
        ) : (
                ...
                <div key={cat.category} className="group">
                  <div className="flex justify-between items-end mb-2">
                    <span className="font-medium text-white flex items-center gap-2 text-sm">
                      <span
                        className={`w-2 h-2 rounded-full ${color.bar} shrink-0`}
                  ...
                  <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        boxShadow: `0 0 10px ${color.glow.replace("0.8", "0.4")}`,
                      }}
                    />
                  </div>
                </div>
