...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${STATUS_STYLE[s] ?? STATUS_STYLE.pending}`}
    >
      {s}
    </span>
  );
        ...
        <tbody>
          {rows.map((b: any) => (
            ...
            >
              ...
              <td className="py-3 px-4 text-xs text-white/60 max-w-[140px] truncate">
                {b.location ?? "—"}
              </td>
              <td className="py-3 px-4 text-xs text-white/60">
                {b.guestCount ?? "—"}
        ...
        <tbody>
          {rows.map((b: any) => (
            ...
            >
              ...
              <td className="py-3 px-4">
                <span
                  className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${b.fulfillmentType === "delivery" ? "bg-purple-400/10 text-purple-400" : "bg-white/5 text-white/40"}`}
                >
                  {b.fulfillmentType ?? "—"}
                </span>
              </td>
...
export const AdminBookingsTable: React.FC<Props> = ({
  serviceBookings,
  assetBookings,
  eventBookings,
}) => {
  const [activeTab, setActiveTab] = useState<"service" | "asset" | "event">(
    "service",
  );

  const counts = {
    service: serviceBookings.length,
    asset: assetBookings.length,
  ...
  return (
    <section className="bg-surface-highlight/20 border border-white/5 rounded-[2rem] overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/30 mt-0.5">
              {total} total across services, gears &amp; events
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 bg-black/30 p-1 rounded-xl border border-white/5">
          {TABS.map((tab) => (
            ...
            >
              ...
              {tab.label}
              <span
                className={`ml-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-bold ${activeTab === tab.key ? "bg-white/15 text-white" : "bg-white/5 text-white/30"}`}
              >
                {counts[tab.key]}
              </span>
            </button>
      ...
      <div className="min-h-[200px]">
        {activeTab === "service" && (
          <ServiceBookingsTab rows={serviceBookings} />
        )}
