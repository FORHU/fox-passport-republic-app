...
const ROLE_LABELS: Record<string, string> = {
  eventFoxer: "Host",
  venueFoxer: "Mayor",
  gearFoxer: "Foxer (Asset)",
  serviceFoxer: "Foxer (Service)",
  investor: "Investor",
};
...
function ApplicationDetailDrawer({
  ...
  return (
    <div
      className="fixed inset-0 z-999 flex items-end sm:items-center justify-center sm:justify-end bg-black/70 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
                  ...
                  <div className="bg-white/5 rounded-xl p-4 space-y-3">
                    ...
                    {data.bio && (
                      <div>
                        ...
                        <p className="text-white/80 text-sm leading-relaxed bg-black/20 rounded-lg p-3">
                          {data.bio}
                        </p>
                      </div>
                    )}
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      setApplications((prev) =>
        prev.map((a) =>
          a.id === id ? { ...a, status: "approved" as RoleStatus } : a,
        ),
      );
      setSelectedApp((prev) =>
        prev?.id === id ? { ...prev, status: "approved" as RoleStatus } : prev,
      );
    } catch (err: any) {
