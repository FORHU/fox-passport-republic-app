...
const STUDIO_TABS = ['Space Types', 'Tech & AV', 'Amenities', 'Staffing'] as const;
...
export default function MobileVenueStudio({ venueId }: Props) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(['Rooftop', 'Poolside']));
  ...
  return (
    <div style={{ background: '#050608', minHeight: '100svh', color: '#fff', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <p style={{ flex: 1, fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-display,"Space Grotesk",sans-serif)', margin: 0 }}>Venue Studio</p>
        <button
          style={{ width: 36, height: 36, borderRadius: 999, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
        >
      ...
      <div
        ...
        style={{
          position: 'fixed', top: 126, left: 0, right: 0, zIndex: 4,
        ...
        {STUDIO_TABS.map((tab) => {
          ...
          return (
            <button
              ...
              style={{
                flexShrink: 0, padding: '10px 16px', background: 'none', border: 'none', cursor: 'pointer',
                fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap',
                color: active ? '#ccff00' : 'rgba(255,255,255,0.4)',
                borderBottom: active ? '2px solid #ccff00' : '2px solid transparent',
      ...
      {/* Scrollable grid */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '180px 20px 100px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? `${STRIPE_BG}, rgba(204,255,0,0.06)` : 'rgba(255,255,255,0.03)',
                borderRadius: 14, padding: 12, cursor: 'pointer', textAlign: 'left',
              }}
            >
              <span
                className="material-symbols-outlined"
                style={{ fontSize: 18, color: active ? '#ccff00' : 'rgba(255,255,255,0.35)', flexShrink: 0 }}
              >
                {item.icon}
              </span>
      ...
      <div style={{
        ...
        padding: '16px 20px 30px', borderTop: '1px solid rgba(255,255,255,0.08)',
        background: 'rgba(5,6,8,0.95)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
      }}>
        <button style={{
          width: '100%', background: '#ccff00', color: '#000', fontWeight: 800,
          fontSize: 14, border: 'none', borderRadius: 16, padding: 16, cursor: 'pointer',
        }}>
