...
interface AddGroupMemberModalProps {
  ...
  /** Ids already in the group — excluded from the pickable list. */
...
// group instead of creating one — the only real difference is the exclude
// list (current members can't be re-added) and the mutation it calls.
export function AddGroupMemberModal({
  ...
  const excludeIds = useMemo(
    () => new Set([...existingParticipantIds, userId ?? ""]),
    [existingParticipantIds, userId],
  );
  ...
  const candidates = useMemo(() => {
    ...
    for (const c of conversations) {
      ...
      if (excludeIds.has(c.otherUser.id)) continue;
  ...
  const handleAdd = () => {
    if (selected.size === 0 || addParticipants.isPending) return;
    addParticipants.mutate(
      { conversationId, userIds: Array.from(selected) },
      {
        onSuccess: (conversation) => onAdded(conversation),
        onError: (error: any) =>
          toast.error(
            error?.response?.data?.message || "Could not add to this group.",
          ),
      },
    );
  };
    ...
    >
      <div className="relative flex min-h-screen w-full flex-col bg-zinc-950 sm:min-h-0 sm:max-h-[80vh] sm:w-full sm:max-w-md sm:rounded-2xl sm:border sm:border-zinc-800 sm:shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-zinc-800/80 bg-zinc-950/95 px-4 py-3 backdrop-blur sm:rounded-t-2xl sm:px-5">
          <h2 className="text-sm font-bold text-white">Add Member</h2>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="border-t border-zinc-800/80 p-3 shrink-0">
          ...
          >
            ...
            {selected.size > 0 ? `Add (${selected.size})` : "Add to group"}
          </button>
        </div>
      </div>
    </div>,
