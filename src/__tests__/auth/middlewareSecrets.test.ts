 ...
 * value that verifies a token also mints one. While `proxy.ts` verified
 ...
 * `queryDefaults.test.ts` strips them: the comment in `proxy.ts` that
...
describe("the frontend holds no backend signing key", () => {
  it("proxy.ts does not read ACCESS_TOKEN_SECRET", () => {
    expect(read("proxy.ts")).not.toMatch(/ACCESS_TOKEN_SECRET/);
  });

  it("proxy.ts does not verify the token itself", () => {
 ...
 *
 * The routes are read out of `proxy.ts` rather than restated here, so
