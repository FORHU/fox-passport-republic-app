  ...
  <v-row no-gutters justify="center">
    <v-form v-model="formValid" @submit.prevent ref="formRef" class="my-5 my-md-10 mx-3 mx-md-5 text-secondary w-100"
      ...
      <v-row no-gutters class="w-100 my-7 my-md-10">
        <v-col cols="12" class="text-24px font-700">Create an account as user</v-col>
        <v-col cols="12" class="text-16px text-subtitle">Already have an account? <NuxtLink
            :to="{ name: 'country-login-user', params: { country } }" class="text-secondary text-decoration-underline">
            Log
            in as user</NuxtLink></v-col>
      </v-row>
      <v-row no-gutters class="md-flex ga-5 flex-wrap w-100">
        <v-col>
          ...
          <v-text-field v-model="form.first_name" variant="outlined" placeholder="John" density="comfortable"
            rounded="lg" class="mt-1" :rules="[(v: string) => requiredInputForName(v)]"></v-text-field>
        </v-col>
        <v-col>
          ...
          <v-text-field v-model="form.last_name" variant="outlined" placeholder="Doe" density="comfortable" rounded="lg"
            class="mt-1" :rules="[(v: string) => requiredInputForName(v)]"></v-text-field>
        </v-col>
      </v-row>
        ...
        <v-col>
          ...
          <v-menu v-model="showPicker" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
            ...
            <v-date-picker v-model="dateOfBirth" @input="dateInput = false" show-adjacent-months elevation="24" no-time
              @update:model-value="handleDateChange" :max="new Date().toISOString().substring(0, 10)"
              placeholder="Pick a date"></v-date-picker>
          </v-menu>
        </v-col>
      </v-row>
      ...
      <v-row no-gutters class="d-flex ga-5 flex-wrap w-100 mt-2">
        <v-col cols="12">
          ...
          <v-text-field v-model="form.password" variant="outlined" type="password" placeholder="+6 characters"
            density="comfortable" rounded="lg" class="mt-1" :rules="passwordRules"></v-text-field>
        </v-col>
      </v-row>
      <v-row no-gutters class="d-flex ga-5 flex-wrap w-100 mt-2">
        <v-col cols="12">
      ...
      <v-row no-gutters class="d-flex ga-5 flex-wrap w-100 mt-7">
        <v-col cols="12">
          <v-btn variant="flat" color="primary" type="submit" size="large" rounded="lg" class="mt-1 w-100 font-500"
            :disabled="!formValid || confirmPassword == ''" @click="onSubmit" :loading="processing">Create Account</v-btn>
        </v-col>
      </v-row>
    </v-form>
