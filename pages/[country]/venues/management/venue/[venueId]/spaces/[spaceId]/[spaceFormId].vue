<template>
    <v-form ref="form" @submit.prevent v-model="formValid" :disabled="mode == 'view'">

        <v-app-bar :elevation="0" style="border-bottom: 2px solid #DEDFE3;">
            <!-- HEADER CREATE MODE -->
            <v-row v-if="mode == 'create' && activeSpacePage" no-gutters class="w-100 px-5">
                <v-row no-gutters justify="center">
                    <v-col cols="12" md="10" lg="8" xl="7" class="d-flex ga-7 align-center">
                        <span style="width: 90%;">
                            <span class="text-16px text-charcoal">Step {{ activeSpacePage + '/6' }} <span
                                    class="text-18px text-secondary font-500">{{ pageItems[activeSpacePage].title
                                    }}</span></span>
                            <span><v-progress-linear :model-value="(activeSpacePage / 6) * 100" color="primary"
                                    height="5" rounded="lg" class="mt-1"></v-progress-linear></span>
                        </span>
                        <span>
                            <v-btn variant="outlined" border="secondary md" size="large" text="Exit Space" rounded="lg"
            ...
            <!-- HEADER UPDATE MODE -->
            <v-row v-else-if="(mode == 'update' || mode == 'view') && activeSpacePage" no-gutters class="w-100 px-5"
                justify="center">
                <v-col cols="12" md="10" lg="8" xl="7" class="d-flex align-center justify-space-between">
                    <v-col v-if="mdAndUp" cols="9" sm="10" lg="auto" class="d-flex justify-start pa-0 ma-0">
                        <v-tabs :model-value="activeSpacePage" align-tabs="center" class="px-0 mx-0" height="50px"
                            selected-class="bg-transparent" density="compact" show-arrows>
                            <template v-for="tabItem in pageItems" :key="tabItem?.title">
                                <v-tab v-if="tabItem?.title" :value="tabItem?.page" :ripple="false" color="secondary"
                                    ...
                                    @click="navigateTo({ name: pageRoute, params: { country, venueId, spaceId, spaceFormId: tabItem?.id } })"
                                    class="text-charcoal text-16px"
                                    :style="{ borderBottom: activeSpacePage == tabItem?.page ? '2px solid rgba(var(--v-theme-secondary)' : '' }">{{
                                        tabItem?.title }}</v-tab>
                            </template>
            ...
            <v-row v-else no-gutters>
                <v-row no-gutters class="w-100 h-100 px-5 px-md-0" justify="center">
                    <v-col cols="12" md="10" lg="8" xl="7" class="py-10">
                        ...
                        <VenueFormSpaceDetails v-if="activeSpacePage === 1" v-model="space"
                            @save-draft="handleUpdateSpace()" />
                        <VenueFormSpacePhotos v-if="activeSpacePage === 2" v-model="space" />
                        <VenueFormSpaceCapacityAndLayouts v-if="activeSpacePage === 3" v-model="space"
                            @save-draft="handleUpdateSpace()" @refresh-space-data="refreshSpaceData"
                            @clear-validation="form.resetValidation()" />
                        <VenueFormSpaceFeatures v-if="activeSpacePage === 4" v-model="space" />
                        <VenueFormSpaceKeywords v-if="activeSpacePage === 5" v-model="space" />
                        <VenueFormSpacePriceAndOpeningHours v-if="activeSpacePage === 6" v-model="space" />
                    </v-col>
                </v-row>
            </v-row>
        </v-main>
                ...
                <v-row no-gutters class="w-100 h-100" justify="center">
                    <v-col cols="12" md="10" lg="8" xl="7" class="">
                        <v-row no-gutters class="w-100 h-100" align="center">
                            <v-col cols="6">
                                <v-btn variant="outlined" border="secondary md" color="secondary"
                                    @click.stop="handlePrevious" rounded="lg" size="large" class=""
                                ...
                                <template v-if="activeSpacePage == 6 && isAdmin">
                                    ...
                                    <div v-if="smAndUp" class="d-flex align-center ga-2 ">
                                        <v-btn type="submit" rounded="lg" size="large" text="Save" color="secondary"
                                            border="secondary md" variant="outlined" class="" :loading="updatingSpace"
                                            @click.stop="saveProgress" />
                                        <v-btn v-if="isForApproval" variant="flat" color="rejected" text="Reject"
                                            size="large" rounded="lg" class="" @click="showRejectPrompt = true"></v-btn>

                                        <v-btn v-if="isAllowSuspend" variant="flat" color="rejected" text="Suspend"
                                            size="large" rounded="lg" class=""
                                            @click="showSuspendPrompt = true"></v-btn>

                                        <v-btn v-if="isForDeletion" color="rejected" variant="flat"
...
-->
    <DialogPromptNew v-model="showDeletePrompt" prompt-title="Are you sure you want to delete this space?"
        prompt-text="Please note that your space won't be listed anymore." agree-button-text="Approve Delete"
        disagree-button-text="No" @disagree="showDeletePrompt = false" @agree="handleApproveDeleteSpace"
        show-header-message agree-button-color="rejected" />

    <DialogPromptNew v-model="showSuspendPrompt" prompt-title="Are you sure you want to suspend this venue?"
        prompt-text="Please note that by proceeding, this venue will be removed from the listing."
        agree-button-text="Suspend" disagree-button-text="Nevermind" @agree="handleAdminAction('SUSPENDED')"
        show-header-message agree-button-color="rejected" />


    <DialogPromptNew v-model="showRejectPrompt" prompt-title="Are you sure you want to reject approval of this venue?"
        prompt-text="Please note that by proceeding, this venue will not be listed." agree-button-text="Reject"
    ...
    <DialogPromptNew v-model="showExitDraftDialog" @agree="handleSave" @disagree="handleExit"
        ...
        disagree-button-text="Exit Page" agree-button-text="Save as draft" />
    <DialogPromptNew v-model="showExitDialog" @agree="handleSave" @disagree="handleExit"
        :prompt-title="`Unsaved Progress`"
        prompt-text="Are you sure to exit this page with an unsaved progress or save your progress?"
        disagree-button-text="Exit Page" agree-button-text="Save Progress" />

    <DialogPromptNew v-model="showNoSelectedFeaturePrompt" @agree="handleProceedToNextPage"
        @disagree="showNoSelectedFeaturePrompt = false" prompt-title="No selected feature"
        prompt-text="Are you sure you want to proceed without selecting any feature?" disagree-button-text="No"
        :agree-button-text="isDRAFT ? 'Proceed to next page' : 'Save & continue'" :loading="updatingSpace" />
</template>
...
const { isVenueMember, isVenueOwner, isVenueAdmin, isAdmin, isUser, isAdminMember, isAdminSales } = useAccess();
...
const { mode, venue, activeSpacePage, updateVenue, isVenuePartOfVenueWithConsent } = useVenue();
...
const pageRoute = "country-venues-management-venue-venueId-spaces-spaceId-spaceFormId";
...
const isAllowPublish = computed(() => {
    const status = space.value.status;
    return status !== "PUBLISHED" && status !== 'FOR_APPROVAL' && status !== 'DELETED';
})
...
const saveProgress = async (type?: 'exit') => {
    ...
    if (activeSpacePage.value == 4) {
        ...
        const obj = space.value.features?.find(x => x.key == '12' && x.reference == 'facilities')
...
const handleAdminAction = async (status: "SUSPENDED" | "PUBLISHED" | "REJECTED") => {
    ...
    try {
        ...
        if (status === "PUBLISHED") {
            ...
            const atLeastOneIsPublished = spaceListArr.some((x: TVenueSpace) => x.status == 'PUBLISHED');
            const noPendingForApproval = !spaceListArr.some((x: TVenueSpace) => x.status == 'FOR_APPROVAL');
...
const handleNext = async () => {
    ...
    if (mode.value === "update" || mode.value === "create") {
        ...
        if (activeSpacePage.value === 2) {
            ...
            const atLeastOneImage = photo_arr?.some(x => x?.contentType?.includes('image'))
            if (!atLeastOneImage) {
                setSnackbar({
...
const handlePageChange = (val: number) => {
    ...
    if (!newFormId) return;
    navigateTo({ name: pageRoute, params: { country, venueId, spaceId, spaceFormId: newFormId } })
}


const handleGoToNextPage = () => {
    if (activeSpacePage.value !== 6) {
        const nextPageParams = pageItems.find(x => activeSpacePage.value && x.page == (activeSpacePage.value + 1))?.id
        ...
        navigateTo({ name: 'country-venues-management-venue-venueId-spaces-spaceId-spaceFormId', params: { country, venueId, spaceId, spaceFormId: nextPageParams } })
    }
...
const handlePrevious = async () => {
    if (activeSpacePage.value > 1) {
        ...
        !isAdmin && await updateSpace(spaceId as string, { form_steps: activeSpacePage.value - 1 })
        const nextPageParams = pageItems.find(x => activeSpacePage.value && x.page == (activeSpacePage.value - 1))?.id
        if (!nextPageParams) return;
        navigateTo(`/${country}/venues/management/venue/${venueId}/spaces/${spaceId}/${nextPageParams}`)
...
const refreshSpaceData = async () => {
    ...
    space.value.pricing.currency = currentCurrency(space.value?.venue?.address?.country).code || '';
    keyComponent.value++;
    setTimeout(() => {
...
const getActivePage = async () => {
    ...
    if (!isAdmin && mode.value == 'create') {
        if (formSteps && formSteps > 1 && formSteps < 7) {
            ...
            if (pageParams) {
                await navigateTo(`/${country}/venues/management/venue/${venueId}/spaces/${spaceId}/${pageParams}`)
            } else {
                await navigateTo(`/${country}/venues/management/venue/${venueId}/spaces/${spaceId}/details`)
            }
