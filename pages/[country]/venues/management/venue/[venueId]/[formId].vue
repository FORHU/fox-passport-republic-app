<template>
    <v-form ref="form" @submit.prevent v-model="formValid" :key="`${activePage}-${venueKey}`" class=""
        ...
        <v-app-bar :elevation="0" style="border-bottom: 2px solid #DEDFE3;">
            ...
            <v-row v-if="mode == 'create' && activePage" no-gutters class="w-100 px-5">
                <v-row no-gutters justify="center">
                    <v-col cols="12" md="10" lg="8" xl="7" class="d-flex ga-7 align-center">
                        <span style="width: 90%;">
                            <span v-if="activePage" class="text-16px text-charcoal">Step {{ activePage + `/7` }}
                                <span class="text-18px text-secondary font-500">{{ pageItems?.find(x => x?.page ==
                                    activePage)?.title
                                    }}</span></span>
                            <span><v-progress-linear v-if="activePage" :model-value="(activePage / 7) * 100"
            ...
            <!-- HEADER UPDATE MODE -->
            <v-row v-else-if="(mode == 'update' || mode == 'view') && activePage" no-gutters class="w-100 px-5" justify="center">
                <v-col cols="12" md="10" lg="8" xl="7" class="d-flex  align-center justify-space-between pa-0 ma-0">
                    <v-col v-if="mdAndUp" cols="8" sm="10" lg="auto" class="d-flex justify-start pa-0 ma-0">
                        <v-tabs :model-value="activePage" selected-class="bg-transparent" density="compact" show-arrows
                            size="small" height="50px">
                            <template v-for="tabItem in pageItems" :key="tabItem?.title">
                                <v-tab v-if="tabItem?.title" :value="tabItem?.page" :ripple="false" color="secondary"
                                    ...
                                    @click="navigateTo({ name: pageRoute, params: { country, venueId, formId: tabItem?.id } })"
                    ...
                    <v-col v-else class="d-flex align-center">
                        <v-select :model-value="activePage" :items="pageItems.filter(x => x.title !== '')"
            ...
            <v-row v-else no-gutters>
                <v-row no-gutters class="w-100 h-100 px-5 px-md-0" justify="center">
                    <v-col cols="12" md="10" lg="8" xl="7" class="py-5">
                        ...
                        <VenueFormAddressAndLocation v-if="activePage == 2" v-model="venue" />
                        <VenueFormFoodAndBeverages v-if="activePage === 3" v-model="venue" />
                        <VenueFormVenueDetails v-if="activePage === 4" v-model="venue" />
                        <VenueFormVenuePolicyDetails v-else-if="activePage === 5" v-model="venue" />
                        <template v-if="isAdmin">
                            <VenueFormPaymentOptions v-if="activePage === 6" v-model="venue" />
                            <VenueFormVenueSpaces v-if="activePage === 7" />
                            <VenueFormReviewVenue v-if="activePage === 8" v-model="venue"
                                @refresh-venue="handleRefreshVenue"
                                @go-to-spaces="navigateTo({ name: pageRoute, params: { country, venueId, formId: 'spaces' } })" />

                        </template>
                        <template v-else>
                            <VenueFormVenueSpaces v-if="activePage === 6" />
                            <VenueFormReviewVenue v-if="activePage === 7" v-model="venue"
                                @refresh-venue="handleRefreshVenue"
                                @go-to-spaces="navigateTo({ name: pageRoute, params: { country, venueId, formId: 'spaces' } })" />
                        </template>
                    ...
                    <v-col cols="12" md="10" lg="8" xl="7" class="">
                        <v-row no-gutters class="w-100 h-100" align="center">
                            ...
                            <v-col cols="6" class="d-flex justify-end">
                                ...
                                <p v-if="
                                    !atleastOneSpaceFinished && formId == 'spaces' && mode !== 'view' && !isSubscriptionBased
                                " class="text-disabled font-400 text-14px">
                                    Finish adding spaces before you continue
                                </p>
                                <v-btn v-else-if="(isAdmin && activePage !== 8) || (!isAdmin && activePage !== 7)"
                                    :type="mode === 'view' ? 'button' : 'submit'" color="secondary" rounded="lg"
                                    size="large" variant="flat" @click.stop="handleNext" :loading="updatingVenue"
                                    class=""
                                    :text="(mode == 'create' || formId == 'spaces' || mode == 'view') ? 'Next' : 'Save & Continue'" />

                                <template v-if="activePage == 8 && isAdmin">
                                    <v-btn v-if="isForRejection" color="rejected" variant="flat" text="Reject"
                                        size="large" rounded="lg" class="mr-2" @click="showRejectPrompt = true"></v-btn>

                                    <v-btn v-if="isAllowSuspend" color="rejected" variant="flat" text="Suspend"
                                        size="large" rounded="lg" class="mr-2"
                                        @click="showSuspendPrompt = true"></v-btn>

                                    <v-btn v-if="isForDeletion" color="rejected" variant="flat" text="Approve Delete"
                                        size="large" rounded="lg" class="mr-2"
                                        @click="showPermanentlyDeletePrompt = true"></v-btn>

                                    <v-btn v-if="status == 'FOR_APPROVAL'" :disabled="!isForApproval" color="secondary"
                                        variant="flat" text="Approve" size="large" rounded="lg"
    ...
    <DialogDeletePrompt v-model:delete="showPromptDelete" prompt-title="Are you sure you want to delete this venue?"
        prompt-text="Please note that deletion requires approval." agree-button-text="Proceed" disagree-button-text="No"
        @disagree="showPromptDelete = false" @agree="handleDeleteVenue" max-width="500" :showHeaderMessage="true" />
    ...
    <DialogPromptNew v-model="showPermanentlyDeletePrompt" @agree="handlePermanentlyDelete"
        ...
        prompt-text="Please note that this action can't be undone." disagree-button-text="Nevermind"
        agree-button-text="Delete it!" agree-button-color="rejected" />

    <DialogRestorePrompt v-model="showRestorePrompt" prompt-title="Are you sure you want to restore this venue?"
        prompt-text="Note that this venue will be restored under approval section." agree-button-text="Restore"
        disagree-button-text="No" @disagree="showRestorePrompt = false" @agree="handleRestoreVenue"
...
const isForApproval = computed(() => {
    const atLeastOneIsPublished = spaces.value?.some((x: TVenueSpace) => x.status == 'PUBLISHED');
    const noPendingForApproval = !spaces.value?.some((x: TVenueSpace) => x.status == 'FOR_APPROVAL');
...
const atleastOneSpaceFinished = computed(() => {
    ...
    const status = venue.value?.status
    if (
        status == 'FOR_APPROVAL' || status == 'PUBLISHED'
    ) {
        return true;
    } else {
        return false;
...
watchEffect(() => {
    ...
    if (activePage.value == 7) {
        if (!isAdmin && venue.value?.status == "DRAFT" && spaces.value.some((x) => x.status == "FOR_APPROVAL")) {
...
const handlePageChange = (val: number) => {
    ...
    if (!newFormId) return;
    navigateTo({ name: pageRoute, params: { country, venueId, formId: newFormId } })
}
...
const handleGoToNextPage = async () => {
    ...
    if (activePage.value < 8) {
        const nextPageParams = pageItems.find(x => activePage.value && x.page == (activePage.value + 1))?.id
        if (!nextPageParams) return;
        navigateTo(`/${country}/venues/management/venue/${venueId}/${nextPageParams}`)
    }
}
...
const handleUpdateVenue = async (type?: 'save') => {
    ...
    if (!isAdmin && type !== 'save' && mode.value == 'create' && activePage.value && activePage.value < 7) {
...
const getActivePage = async () => {
    ...
    if (!isAdmin && mode.value == "create") {
        if (formStep && formStep > 1 && formStep < 9) {
            ...
            if (pageParams) {
                await navigateTo(`/${country}/venues/management/venue/${venueId}/${pageParams}`)
            } else {
                await navigateTo(`/${country}/venues/management/venue/${venueId}/overview`)
            }
...
const checkMode = async () => {
    ...
    if (isVenueMember || space.value.status == 'DELETED' || (isAdminMember && status !== 'REQUEST_FOR_CONSENT') || isAdminSales) {
        mode.value = 'view';
        return;
    }
    await (isDRAFT.value && !isAdmin) ? mode.value = 'create' : mode.value = 'update'
}
