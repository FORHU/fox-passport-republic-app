        ...
        <template v-if="loggedIn">
          <template v-for="navigationItem in navigation" :key="navigationItem.title" v-if="!smAndDown">
            <nuxt-link :to="{ name: navigationItem.to, params: navigationItem.params }"
              class="d-flex align-center ga-1 font-400 mx-3" style="text-decoration: unset; color: unset">
              ...
              <template v-slot:activator="{ props }">
                <v-btn class="custom-outline button-with-icon" block height="40px" variant="outlined" v-bind="props"
                  ...
                  <v-row class="w-100">
                    ...
                    <v-img src="/public/svg/people.svg" height="18px" width="30px"></v-img>
                  </v-row>
                </v-btn>
              </template>
                ...
                <v-row>
                  <v-col cols="12">
                    <v-text-field type="number" min="0" append-icon="" persistent-placeholder
                      placeholder="Number of guest" prepend-inner-icon="mdi-account-multiple-outline" @keydown="preventNegative"
                  ...
                  <v-col cols="6" v-if="!showGuestFilter">
                    <v-btn variant="text" class="text-primary font-400" @click="clearFilter('guest')">Clear</v-btn>
                  </v-col>
              ...
              <template v-slot:activator="{ props }">
                <v-btn block class="custom-outline custom-date-btn" variant="outlined" height="40px" v-bind="props"
                  ...
                  <v-row no-gutters justify="space-between">
                    ...
                    <v-img src="/public/svg/calendar.svg" height="18px" width="30px"></v-img>
                  </v-row>
                </v-btn>
              </template>
              <v-card width="650" height="370" class="d-flex justify-center" variant="text">
                <v-row no-gutters>
                  ...
                  <v-col cols="6" class="mt-15 d-flex justify-start px-2">
                    <v-btn variant="text" class="text-primary font-400" @click="clearFilter('date')">Clear</v-btn>
                  </v-col>
            ...
            <v-menu :close-on-content-click="false" v-model="isPriceMenuOpen" class="font-400">
              ...
              <v-card width="400" height="280">
                <v-row no-gutters>
                        ...
                        <v-card variant="outlined" height="33" class="d-flex justify-center">
                          <v-row no-gutters height="200">
                            ...
                            <v-col cols="8" class="ml-1">
                              <v-text-field type="number" min="0" placeholder="0" variant="plain" @update:model-value="handleUpdatePriceValueIndexZero
                      ...
                      <v-col cols="12">
                        <v-card variant="outlined" height="33" class="d-flex justify-center">
                          <v-row no-gutters height="200">
                            ...
                            <v-divider vertical></v-divider>
                            <v-col cols="8" class="ml-1">
                              <v-text-field type="number" min="0" max="10000" placeholder="1000+" variant="plain"
                                @update:model-value="handleUpdatePriceValueIndexOne
                  ...
                  <v-col cols="12" class="pa-5">
                    <v-range-slider @update:model-value="handleUpdatePriceValue" strict v-model="priceValue" step="10"
                      :max="10000" :min="0"></v-range-slider>
                  </v-col>
                  <v-divider></v-divider>
                  <v-col cols="6" class="mt-5 d-flex justify-start px-2">
                    <v-btn variant="text" class="text-primary" @click="clearFilter('price')">Clear</v-btn>
                  </v-col>
    ...
    <v-row no-gutters class="px-2 pr-6 pt-5">
      <v-col cols="12" class="left">
        <v-row v-if="loader" no-gutters class="w-100">
          <v-col cols="12" sm="12" md="7" lg="7" class="d-flex flex-wrap justify-center justify-md-start ga-2">
            <template v-for="item in 8" :key="item">
              <v-skeleton-loader class="ma-6" width="300" type="card, paragraph"></v-skeleton-loader>
            </template>
          </v-col>
        </v-row>
        <v-row v-else>
          <v-col cols="12" sm="12" md="7" lg="7" class="left" v-if="
            (smAndDown &&
              showingMethod == 'List' &&
              showingMethod != 'Map') ||
            !smAndDown
          ">
            <v-row class="pl-6 font-400 pt-4 pb-3" v-if="venues.length > 0">
              <span class="font-600">
                {{
                  `${venues.length} ${venues.length > 1 ? "results" : "result"} for ${eventType} ${venues.length > 1 ?
                    "Venues" : "Venue"}`
                }}
              </span>
            </v-row>
            ...
            <div class="scrollable-content">
              <v-row class="pl-4">
                ...
                <v-col cols="12" sm="6" md="6" lg="4" xl="3" v-for="(venue, index) in venues" :key="index">
                  <CardSpotlightCard :space="venue" :imageSrc="imageSrc" :numGuest="numGuest"
                    :date_calendar="date_calendar" :event_type="eventType" :navigateTo="navigateTo" :country="country"
                    :favoriteCount="favoriteCount" @folderCreated="folderCreated" :favoriteObj="favoriteObj"
                    :priceFilter="priceValue" @deselectFavorite="deselectFavorite" />
                </v-col>
              </v-row>
            </div>

            <v-col cols="12" :class="smAndDown ? 'mb-16' : 'fixed-pagination'">
              <v-pagination v-if="venues.length > 0 && totalPages > 1" v-model="page"
          ...
          <!-- Maps -->
          <v-col cols="12" sm="12" md="5" lg="5" class="right pa-0 ma-0" v-if="
            (smAndDown &&
              showingMethod == 'Map' &&
              showingMethod != 'List') ||
            !smAndDown
          ">
            <!-- <v-row class="h-50 pt-3"> -->
            <MapGoogleMap :country="venueLocation" :coordinates="coordinates" :showDetails="true" :mobile="false" />
      ...
      <v-row>
        <v-snackbar style="z-index: 1200" @click.stop="changeShowMethod()" rounded="pill" :timeout="-1"
          min-width="100px" v-model="snackbar" class="hidden-md-and-up mb-4">
          <v-row>
            <v-col v-if="showingMethod == 'List'" class="d-flex mr-2">
              <v-img src="/public/svg/map.svg" height="20px" width="20px"></v-img>
              <span class="text-14px">Map</span>
            </v-col>

            <v-col v-if="showingMethod == 'Map'" class="d-flex mr-2">
              <v-img src="/public/svg/list.svg" height="20px" width="20px"></v-img>
      ...
      <!-- <v-col> -->
      <v-dialog style="z-index: 1199" v-model="mobileMap" fullscreen width="100%" height="100%">
        <MapGoogleMap :country="venueLocation" :coordinates="coordinates" :showDetails="true" :mobile="true"
