...
const ExperienceListing: React.FC<ExperienceListingProps> = ({
  ...
  return (
    <div
      ...
      } gap-6 items-start p-4 bg-gray-50/30 border border-gray-100 rounded-[2.5rem]`}
    >
      {/* Main Image Block */}
      <div
        className={`relative ${
          isVertical ? "md:col-span-1 h-[500px] md:h-[600px]" : "md:col-span-1 h-[400px]"
        } rounded-[2rem] overflow-hidden shadow-lg group`}
      >
        <Link href={`/venues/${experience.id}`}>
          <img
