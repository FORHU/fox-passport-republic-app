    ...
    <v-row no-gutters class="pa-5 pa-md-7 w-100" style="height:max-content">
            ...
            <v-row no-gutters class="pa-2" style="position: absolute;">
              ...
              <div>
                    <!-- <RatingCardView
                      ...
                      :total-reviews="ratingDetails?.totalReviews"
                      :enquiry="enquiry"
                    /> -->
              </div>
            </v-row>
      ...
      <v-row no-gutters class="w-100 flex flex-column ga-1 mb-15">
            <v-row no-gutters class="d-flex flex-column sm:flex-row pt-5">
              <v-row
                no-gutters
                class="w-100 h-full d-flex flex-column justify-center"
              >
                <span
                  no-gutters
                  class="font-500 text-16px text-hover text-secondary mb-2"
            ...
            <v-col cols="12" v-if="!bookingConfirmed && !bookingHappened && !bookingCancelled && !bookingArchived"
              ...
              <v-row justify="end" class="w-100 pt-2" no-gutters>
                <v-btn text='Archive Inquiry' variant='text' density='compact' class="text-decoration-underline" @click="emit('archive-inquiry')" />
              </v-row>
            </v-col>
              ...
              <v-row
               v-else-if="isUser"
                no-gutters
                class="d-flex flex-column justify-center w-100 pt-2"
              >
                <v-btn
                  v-if="newEnquiry"
                  variant="flat"
                  ...
                  class="rounded-md"
                  block
                  @click="emit('request-to-book')"
                  >Request To Book</v-btn
                >
  
                <v-btn
                  v-if="bookingCancelled"
                  variant="outlined"
                  block
                  @click="emit('view-cancellation-details')"
                  >View Cancellation Details</v-btn
                >
                ...
                <v-btn
                  v-if="paymentFailed || paymentInProgress"
                  variant="outlined"
                  block
                  size="large"
                  @click="emit('payment-failed')"
                  >Go to Payment</v-btn
                >
                <!-- PAYMENT FAILED/INPROGRESS -->
            ...
            <v-row no-gutters class="">
              ...
              <EnquiryChatInfoRow
                label="Space Name"
                :value="enquiry?.space?.name || ''"
                link
                @link-click="emit('go-to-space')"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Venue Name"
                :value="enquiry?.venue?.name || ''"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Address"
                :value="formatAddress(enquiry?.venue?.address) || ''"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Date"
                :value="longDateFormat(enquiry?.date?.date) || ''"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Check-in"
                :value="convertTimeToAMPM(enquiry?.date?.from) || ''"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Check-out"
                :value="convertTimeToAMPM(enquiry?.date?.to) || ''"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                label="Guests"
              ...
              <EnquiryChatInfoRow
                v-if="enquiry?.flexible_time"
                label="Flexible on date/time"
                :value="'Yes'"
                with-bottom-divider
              />
              <EnquiryChatInfoRow
                ...
                label="Catering"
                :value="cateringOptions ? cateringOptions : 'Required' "
                with-bottom-divider
              />
              <EnquiryChatInfoRow
            ...
            <v-row no-gutters class="w-100">
              <span class="text-subtitle-1 font-500 my-5"
                >In this conversation</span
              >
              <v-row
                no-gutters
                class="w-100 d-flex ga-2 ga-md-3 align-center pb-5"
              >
                <span>
                  <ProfileAvatar
