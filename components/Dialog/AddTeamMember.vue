    ...
    <v-card rounded="lg">
      ...
      <v-card-text>
        ...
        <v-form ref="form" v-model="valid">
          ...
          <v-checkbox-group v-if="venueSelection === 'select'" multiple>
...
const { inviteTeamMember, rolesOptions, getVenueDetails } = useTeamMembers();
...
const sendInvitation = async () => {
  ...
  try {
    const res =  await inviteTeamMember(invitation);
    if (res){
      closeDialog();
    ...
    if (err.code == 'USER_IS_ALREADY_A_MEMBER') {
      setSnackbar({
        color: "error",
