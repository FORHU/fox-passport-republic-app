...
- **Frontend:** A "Join" / "Register" action on `app/event/[eventId]` for templates flagged public, distinct from the existing booking-request flow. The Phase 1 sharing work (OG fix, share/invite UI, QR) already shipped 11 Sep — see `NEXT.md` — so this just needs to hang off it, not build it.
