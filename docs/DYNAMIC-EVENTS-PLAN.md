...
- **Open decision:** the 5 legacy `EventCategory` values (`corporate`, `birthday`, `wedding`, `social`, `other`) need an explicit mapping to the new Category/Subcategory tree decided before this seed step runs — not left implicit — so nothing already in the database ends up orphaned.
...
> **Before starting Phase 1:** Get explicit sign-off on Decision Point Zero (Option B) and its refinement in §3a (a new `EventRegistration` domain, not a reuse of `BookingAttendee`) — currently inferred from this doc being marked Active, not separately confirmed. Everything downstream branches from those two calls. The Phase 1 sharing/OG-preview work already shipped 11 Sep, in parallel — it had no dependency on this plan at all.
