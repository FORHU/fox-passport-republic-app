...
| AUTH-02 | 🟠 | **Login bypasses the proxy** — the browser posts credentials straight to the API from its own axios instance. The one authenticated-concern request crossing origins, and the one that establishes the session. | app |
| AUTH-03 | 🟠 | **Cookie policy lives in the app, not the API** — the API sets no cookies; the app composes them and hand-matches lifetimes the API owns. | both |
| AUTH-04 | 🟡 | **`SESSION_MAX_AGE` written out three times**, each commented "must match `REFRESH_TOKEN_EXPIRY`" — a value in the API's environment. They have disagreed before. | app |
...
### 🟠 Fix soon
- [ ] **AUTH-02** — Route login through `/api/proxy/*`
- [ ] **AUTH-03** — Move cookie authorship to the API (fix the `Set-Cookie` relay first)

### 🟡 Fix when convenient
- [ ] **AUTH-05** — Revoke prior sessions on Google sign-in
- [ ] **AUTH-04** — Delete the duplicated `SESSION_MAX_AGE` — **last** of the
      cookie work, because AUTH-03 dissolves most of it
