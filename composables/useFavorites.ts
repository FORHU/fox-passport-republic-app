...
export function useFavorites(){
  ...
  const addNewFavoriteFolder = async (space_id: string, folder_name: string) => {
  ...
  const addNewFavoriteAToExistingFolder = async (space_id: string,favorite_folder_id:string) => {
    return await useAPI("/v1/favorite", {
      method: "POST",
      body: JSON.stringify({
        space_id: space_id,
    ...
    const updateFavoritesFolder = async ( favorite_folder_id: string, favoriteId: string) => {
    ...
    const updateFavoritesFolderName = async (spaceId: string, folder_name: string, favoriteId: string) => {
           return await useAPI(`/v1/favorite/assign-folder/${favoriteId}`, {
      method: "PATCH",
      body: JSON.stringify({
        space_id: spaceId,
  ...
  const getSpacesOnFolderFavorite = async (page: number, limit: number, folder_id: string) => {
  ...
  const renameFavoriteFolder = async (favorite_folder_id: string,folder_name :string) => {
