...
import { DashboardHeader, ServicesSection } from "@/components/host/dashboard";
import { STATUS_OPTIONS, type ServiceItem } from "@/data/dashboardData";
import { useHostServicesViewAll } from "@/hooks/dashboards/useHostServicesViewAll";
...
export default function HostServicesViewAllPage() {
  const router = useRouter();
  const [searchQuery, setSearchQuery] = useState("");
  const [status, setStatus] = useState<"all" | string>("all");

  const { services: fetchedServices, isLoading, error } =
    useHostServicesViewAll();
  const [services, setServices] = useState<ServiceItem[]>(fetchedServices);
            ...
            {isLoading ? (
              ...
              <div className="glass-panel rounded-[2rem] p-12 border border-red-500/20">
                <div className="flex flex-col items-center gap-3 text-center">
                  ...
                  <div className="text-sm font-bold">Failed to load services</div>
