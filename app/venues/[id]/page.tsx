...
const CustomExperienceBuilder: React.FC<{ isOpen: boolean; onClose: () => void; venuePrice: number }> = ({ isOpen, onClose, venuePrice }) => {
  ...
  const getFilteredServices = () => {
    const CategoryServices = CUSTOM_SERVICES[activeCategory as keyof typeof CUSTOM_SERVICES];
...
const EventDetailsPage: React.FC = () => {
  const router = useRouter(); // Adapted from useNavigate
  const { id } = useParams(); // Adapted from useParams
  const eventId = id; // Adapted
  ...
  const handleDateClick = (day: number) => {
    // Only allow selecting available days (mock logic: day > 10)
    if (day <= 10) return;
    ...
    if (checkInDate === null) {
      ...
      if (day > checkInDate) {
        ...
        // Reset if clicking before check-in or same day
        setCheckInDate(day);
  ...
  const nights = checkInDate && checkOutDate ? checkOutDate - checkInDate : 0;
                 ...
                 <div className="bg-surface-highlight/30 rounded-2xl p-6 border border-white/5">
                    ...
                    <div className="grid grid-cols-7 gap-y-2 text-center text-sm font-medium">
                        ...
                        {[...Array(31)].map((_, i) => {
                            ...
                            const isInRange = checkInDate !== null && checkOutDate !== null && day > checkInDate && day < checkOutDate;
                            const isSelected = isRangeStart || isRangeEnd;
                            
                            return (
                                <div key={day} className="relative py-1" onClick={() => handleDateClick(day)}>
                    ...
                    <div className="flex justify-end mt-6">
                        <button onClick={() => { setCheckInDate(null); setCheckOutDate(null); }} className="text-xs font-bold text-white underline decoration-white/30 hover:decoration-white transition-all">Clear dates</button>
                    </div>
                 </div>
              </div>
                ...
                <div className="glass-card rounded-2xl border border-white/10 p-6 shadow-glow relative overflow-hidden">
                  ...
                  <div className="border border-white/20 rounded-xl overflow-hidden mb-4 relative z-10">
                    <div className="grid grid-cols-2 border-b border-white/20">
                      <div className="p-3 border-r border-white/20 hover:bg-white/5 cursor-pointer transition-colors relative group">
                        ...
                        <div className="text-sm text-white font-medium">{checkInDate ? `Oct ${checkInDate}` : '-'}</div>
                      </div>
                      <div className="p-3 hover:bg-white/5 cursor-pointer transition-colors group">
                        ...
                        <div className="text-sm text-white font-medium">{checkOutDate ? `Oct ${checkOutDate}` : '-'}</div>
                      </div>
                    </div>
                    <div className="p-3 hover:bg-white/5 cursor-pointer transition-colors group">
                  ...
                  <div className="space-y-3 text-sm text-gray-300 relative z-10 pb-4">
                    <div className="flex justify-between">
                      <span className="underline decoration-dotted cursor-pointer">₱{venue.price.toLocaleString()} x {nights} nights</span>
                      <span>₱{(venue.price * nights).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  <div className="flex justify-between items-center text-white font-bold text-lg relative z-10">
                    <span>Total</span>
                    <span>₱{(venue.price * nights + 150).toLocaleString()}</span>
                  </div>
                </div>
